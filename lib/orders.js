const PLANS = {
  ...
  print: {
    ...
    stripeName: "652 комбинации — печатена книшка",
    amount: 99900,
    price: 999,
    label: "999 ден.",
    physical: true
  ...
  komplet: {
    id: "komplet",
    name: "PDF + печатено",
    stripeName: "652 комбинации — PDF и печатена книшка",
    amount: 129900,
    price: 1299,
    label: "1.299 ден.",
    physical: true
...
function parseOrder(body) {
  ...
  if (plan.physical && (order.city.length < 2 || order.address.length < 4)) {
    return { error: "За печатената книшка ни треба град и адреса." };
  }
...
function allowedOrigin(origin) {
  ...
  try {
    ...
    const ours = host === "svet-na-decata.vercel.app" || host.endsWith(".vercel.app");
