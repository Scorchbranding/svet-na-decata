...
function denarLabel(price) {
  return String(price).replace(/\B(?=(\d{3})+(?!\d))/g, ".") + " ден.";
}

function applyDiscount(plan, code) {
  const entered = clean(code, 40).toLowerCase();
  if (!entered) return { plan: plan };
  const expected = String(process.env.DISCOUNT_CODE || "").trim().toLowerCase();
  const percent = Number(process.env.DISCOUNT_PERCENT || "99");
  if (!expected || entered !== expected || !Number.isFinite(percent)) {
    return { error: "Кодот за попуст не важи." };
  }
  const off = Math.min(99, Math.max(1, Math.round(percent)));
  const price = Math.max(1, Math.round(plan.price * (100 - off) / 100));
  return {
    plan: Object.assign({}, plan, {
      price: price,
      amount: price * 100,
      label: denarLabel(price)
    }),
    discountPercent: off
  };
}
...
function parseOrder(body) {
  ...
  const discounted = applyDiscount(plan, body.code);
  if (discounted.error) return discounted;
  const priced = discounted.plan;
  order.price = priced.price;
  order.label = priced.label;
  if (discounted.discountPercent) order.discountPercent = discounted.discountPercent;
  return { plan: priced, order };
}
